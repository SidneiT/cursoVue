<template>
  <div class="door-area">
    <div class="door-frame" :class="{selected:selected && !open}">
      <Gift v-if="open && hasGift" />
    </div>
    <div class="door" :class="{open}" @click="selected=!selected && !open">
      <div class="number" :class="{selected}">{{number}}</div>
      <div class="knob" @click.stop="open=true" :class="{selected}"></div>
    </div>
  </div>
</template>

<script>
import Gift from "./Gift";

export default {
  name: "Door",
  components: { Gift },
  props: {
    number: {},
    hasGift: {
      type: Boolean,
      default: false
    }
  },
  data() {
    return {
      open: false,
      selected: false
    };
  }
};
</script>

<style>
:root {
  --door-area: 5px solid brown;
  --selected-border: yellow;
}

.door-area {
  position: relative;
  width: 200px;
  height: 310px;
  border-bottom: 10px solid #aaa;
  margin-bottom: 20px;
  font-size: 3em;
  display: flex;
  justify-content: center;
  user-select: none;
}

.door-frame {
  position: absolute;
  height: 300px;
  width: 180px;
  border-left: var(--door-area);
  border-top: var(--door-area);
  border-right: var(--door-area);
  display: flex;
  justify-content: center;
  align-items: flex-end;
}

.door {
  position: absolute;
  top: 5px;
  height: 295px;
  width: 170px;
  background-color: chocolate;
  display: flex;
  flex-direction: column;
  align-items: center;
  padding: 20px;
}

.door .knob {
  height: 20px;
  width: 20px;
  border-radius: 50%;
  background-color: brown;
  align-self: flex-start;
  margin-top: 60px;
}

.door-frame.selected {
  border-color: var(--selected-border);
}

.door > .number.selected {
  color: var(--selected-border);
}
.door > .knob.selected {
  background-color: var(--selected-border);
}

.door.open {
  background-color: #0007;
}

.door.open > * {
  display: none;
}
</style>
